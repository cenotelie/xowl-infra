...
public class BaseRDFTest {
     ...
     */
    protected void testPositiveEntailment(String input, EntailmentRegime regime, String result) throws Exception {
                ...
                try {
                    if (result == null) {
                        ...
                        Iterator<? extends Quad> iterator = transaction.getDataset().getAll(
                                null,
                                transaction.getDataset().getIRINode(Vocabulary.xowlStatus),
                                transaction.getDataset().getIRINode(Vocabulary.xowlInconsistent));
                        ...
                        for (Quad q : expected) {
                            Assert.assertTrue("Failed to entail " + q, transaction.getDataset().count(null, q.getSubject(), q.getProperty(), q.getObject()) > 0);
                        }
     ...
     * @param result The expected result, or null if inconsistency shall be detected
     */
    protected void testNegativeEntailment(String input, EntailmentRegime regime, String result) throws Exception {
        SinkLogger logger = new SinkLogger();
        try (RepositoryRDF repositoryRdf = new RepositoryRDF()) {
            repositoryRdf.runAsTransaction((repository, transaction) -> {
                repository.getIRIMapper().addRegexpMap("http://www.w3.org/2013/rdf-mt-tests/(.*)", ResourceAccess.SCHEME_RESOURCE + "/org/w3c/rdf-mt/\\1");
                ...
                try {
                    if (result == null) {
                        ...
                        List<Quad> expected = load(result);
                        for (Quad q : expected) {
                            Assert.assertFalse("Incorrectly entailed " + q, transaction.getDataset().count(null, q.getSubject(), q.getProperty(), q.getObject()) > 0);
                        }
                        W3CTestSuite.matchesQuads(expected, tested);
                    }
                } catch (UnsupportedNodeType exception) {
     ...
     */
    private List<Quad> load(String resource) throws Exception {
        SinkLogger logger = new SinkLogger();
        try (RepositoryRDF repositoryRdf = new RepositoryRDF()) {
            return repositoryRdf.runAsTransaction((repository, transaction) -> {
                repository.getIRIMapper().addRegexpMap("http://www.w3.org/2013/rdf-mt-tests/(.*)", ResourceAccess.SCHEME_RESOURCE + "/org/w3c/rdf-mt/\\1");
