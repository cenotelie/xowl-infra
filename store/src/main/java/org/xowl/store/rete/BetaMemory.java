...
public class BetaMemory implements TokenHolder {
     ...
     */
    private Map<Token, Map<Triple, Token>> mapAscendants;
    /**
     ...
     */
    public void activate(Token token, Triple fact) {
     ...
     */
    private Token buildToken(Token token, Triple fact) {
        Token childToken = new Token(token);
        for (Binder binder : binders)
            binder.execute(childToken, fact);
        Map<Triple, Token> subMap = mapAscendants.get(token);
        if (subMap == null) {
     ...
     * @param fact  The input fact
     */
    public void deactivate(Token token, Triple fact) {
     ...
     */
    private Token resolveToken(Token token, Triple fact) {
        Map<Triple, Token> subMap = mapAscendants.get(token);
    ...
    private void updateTokens(Collection<Binder> binders) {
        for (Token base : mapAscendants.keySet()) {
            Map<Triple, Token> subMap = mapAscendants.get(base);
            for (Entry<Triple, Token> entry : subMap.entrySet())
                for (Binder binder : binders)
