...
class FactCollection implements Collection<Quad> {
    ...
    public boolean contains(Object o) {
        if (!(o instanceof Quad))
            return false;
        Quad quad = (Quad) o;
        if (pattern.getGraph().getNodeType() != VariableNode.TYPE && pattern.getGraph() != quad.getGraph())
            return false;
        if (pattern.getSubject().getNodeType() != VariableNode.TYPE && pattern.getSubject() != quad.getSubject())
            return false;
        if (pattern.getProperty().getNodeType() != VariableNode.TYPE && pattern.getProperty() != quad.getProperty())
            return false;
