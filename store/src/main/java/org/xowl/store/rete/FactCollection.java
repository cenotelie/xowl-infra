...
class FactCollection implements Collection<Quad> {
    ...
    public boolean contains(Object o) {
        if (!(o instanceof Quad))
            return false;
        Quad quad = (Quad) o;
        if (pattern.getGraph().getNodeType() != Node.TYPE_VARIABLE && pattern.getGraph() != quad.getGraph())
            return false;
        if (pattern.getSubject().getNodeType() != Node.TYPE_VARIABLE && pattern.getSubject() != quad.getSubject())
            return false;
        if (pattern.getProperty().getNodeType() != Node.TYPE_VARIABLE && pattern.getProperty() != quad.getProperty())
            return false;
        if (pattern.getObject().getNodeType() != Node.TYPE_VARIABLE && pattern.getObject() != quad.getObject())
            return false;
