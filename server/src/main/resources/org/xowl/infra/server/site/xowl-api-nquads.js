...
function parseNQuads(content) {
	...
	for (var i = 0; i != tokens.length; i++) {
		...
		var item = key in result ? result[key] : { id: key, isIRI: subject.type === "uri", properties: [], graph: graph.value };
...
function lexNQuads(content) {
	...
	for (var i = 0; i < content.length; i++) {
		...
		if (c === '<') {
			...
			var tokenIRI = { type: "uri", value: content.substring(i + 1, j) };
			...
			var tokenLiteral = { type: "literal", value: lexicalValue, "xml:lang": langTag, datatype: datatype };
			...
			var tokenBlank = { type: "bnode", value: content.substring(i + 2, j) };
			result.push(tokenBlank);
			i = j - 1;
		} else if (c === '?') {
			// start of a variable node
			var j = i + 1;
			while (j < content.length && content.charAt(j).trim().length !== 0) {
				j++;
			}
			var tokenBlank = { type: "variable", value: content.substring(i + 1, j) };
			result.push(tokenBlank);
			i = j - 1;
		} else {
